    ...
    checked ? "checked" : ""
      ...
      document.querySelectorAll(".datatable-checkbox").forEach((checkbox) => {
        checkbox.addEventListener("click", (e) => {
          ...
          // Manejo del array valor
          if (!valor.includes(id)) {
            valor.push(id);
            if (valor.length > 1) {
              valor.shift(); // Elimina el primer elemento
            }
          }


          // Manejo del checkbox seleccionado
          if (element.checked) {
            const item = this.getItem(id);
            this.selected.push(item);

            // Si ya hay un elemento seleccionado, eliminar el anterior
            if (this.selected.length > 1) {
              this.removeSelected(valor[0]);
              // Actualiza el checkbox correspondiente
              document.querySelector(
                `.datatable-checkbox[data-id="${valor[0]}"]`,
              ).checked = false;
            }
          } else {
            this.removeSelected(id);
            valor = valor.filter((v) => v !== id); // Elimina el ID del array si se deselecciona
          }

          // Actualizar el estado de los checkboxes
          document.querySelectorAll(".datatable-checkbox").forEach((cb) => {
            cb.checked = valor.includes(cb.getAttribute("data-id"));
          });
